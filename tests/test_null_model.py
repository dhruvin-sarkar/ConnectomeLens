...
from pipeline.null_model import empirical_p_value, rewire
...
def test_empirical_p_value_counts_ties_and_never_reaches_zero():
    null = np.array([0.1, 0.2, 0.3, 0.3])
    assert empirical_p_value(0.3, null) == pytest.approx(3 / 5)
    assert empirical_p_value(0.9, null) == pytest.approx(1 / 5)
